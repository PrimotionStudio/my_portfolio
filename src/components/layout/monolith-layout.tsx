  ...
  return (
    <header className="flex justify-between items-center px-6 py-2 w-full fixed top-0 z-50 bg-[#131313] opacity-40 backdrop-blur-md border-b border-[#abc7ff]/15">
      <div className="flex items-center gap-4">
        <span className="font-mono font-bold text-[#abc7ff] tracking-widest text-xs md:text-sm">MONOLITH_OS_v1.0.4</span>
        <nav className="hidden md:flex gap-6 items-center">
          <Link href="/" className="font-headline tracking-tighter uppercase text-xs text-[#abc7ff]/60 hover:text-[#ecffe3] transition-colors duration-200">System</Link>
          <Link href="/projects" className="font-headline tracking-tighter uppercase text-xs text-[#abc7ff]/60 hover:text-[#ecffe3] transition-colors duration-200">Projects</Link>
          <Link href="/blog" className="font-headline tracking-tighter uppercase text-xs text-[#abc7ff]/60 hover:text-[#ecffe3] transition-colors duration-200">Logs</Link>
        </nav>
      </div>
      <div className="flex items-center gap-3">
        <span className="material-symbols-outlined text-[#abc7ff] text-sm cursor-terminal">minimize</span>
        <span className="material-symbols-outlined text-[#abc7ff] text-sm cursor-terminal">fullscreen</span>
        <span className="material-symbols-outlined text-[#abc7ff] text-sm cursor-terminal">close</span>
      </div>
    </header>
  );
};
  ...
  return (
    <aside className="fixed left-0 top-12 h-screen flex flex-col pt-8 bg-[#131313] w-64 border-r border-[#abc7ff]/15 z-40 hidden md:flex">
      <div className="px-6 mb-8">
        <h2 className="text-[#abc7ff] font-bold font-mono text-sm uppercase tracking-tight">FILE_EXPLORER</h2>
      ...
      <div className="mt-auto p-6 mb-16">
        <div className="bg-surface-container-low p-3 border border-outline-variant/15">
          ...
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-secondary shadow-[0_0_8px_rgba(236,255,227,0.5)]"></span>
      ...
      {navItems.map((item) => (
        <Link key={item.path} href={item.path}>
          {item.isPrimary ? (
            ...
            <span className={`material-symbols-outlined ${pathname === item.path ? 'text-primary' : 'text-[#abc7ff]/50'}`}>{item.icon}</span>
          )}
        </Link>
      ))}
    </nav>
  ...
  return (
    <footer className="fixed bottom-0 w-full flex justify-between items-center px-4 py-1 z-50 bg-[#131313] text-[#ecffe3] font-mono text-[10px] uppercase opacity-80 border-t border-[#abc7ff]/15">
      <div className="flex gap-4">
        <span className="hidden sm:inline">© 2024 SYSTEM_ARCHITECT</span>
...
export const MonolithLayout = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className="min-h-screen bg-background text-on-surface">
      <TopAppBar />
      <SideNavBar />
      <main className="md:ml-64 pt-16 pb-24 md:pb-12 px-6 md:px-12 min-h-screen">
        {children}
      </main>
      <MobileBottomNav />
      <Footer />
